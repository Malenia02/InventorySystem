...
if (isset($_POST['add_product'])) {
    ...
    if (!$userID) {
        // session missing or user not logged in
        error_log("ADD PRODUCT ERROR: User ID not found in session. stock_in will be NULL!");
        echo json_encode([
            'success' => false,
    ...
    try {
        ...
        $photoPath = null; // fallback if upload fails
        error_log("Photo upload failed for product '$name': " . $e->getMessage());
    }
    ...
    try {
        $productId = ProductController::addProduct(
            ...
            $photoPath,
            $sku,
            $userID // ✅ Pass the session user ID
        );
        ...
        echo json_encode([
            'success' => false,
            'error' => 'Failed to add product. See server logs for details.'
        ]);
        exit;
    }
}
...
$old = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    if ($old['product_name'] !== $name)
        $changes[] = "Name: {$old['product_name']} → $name";

    if ($old['sku'] !== $sku)
        $changes[] = "SKU: {$old['sku']} → $sku";

    if ((float)$old['price'] !== $price)
        $changes[] = "Price: {$old['price']} → $price";

    if ((float)$old['sale_price'] !== (float)$sale_price)
        $changes[] = "Sale Price: {$old['sale_price']} → $sale_price";

    if ((int)$old['vatable'] !== $vatable)
        $changes[] = "Vatable: {$old['vatable']} → $vatable";

    if ((int)$old['reorder_level'] !== $reorder)
        $changes[] = "Reorder Level: {$old['reorder_level']} → $reorder";

    if ($old['category_id'] != $category)
    $changes[] = "Category: {$old['category_name']} → {$product['category_name']}";

if ($old['supplier_id'] != $supplier)
    $changes[] = "Supplier: {$old['supplier_name']} → {$product['supplier_name']}";
    ...
    // ============================
    if (!empty($changes)) {
        $message = ("<b>{$old['product_name']}</b> was updated:<br>" . implode("<br>", $changes));
    } else {
